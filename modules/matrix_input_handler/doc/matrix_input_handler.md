...
| 信号 | 方向 | 位宽 | 说明 |
|------|------|------|------|
| `clear_request` | 输出 | 1 | 清除请求信号 |
| `clear_done` | 输入 | 1 | 清除完成标志 |
| `clear_matrix_id` | 输出 | 3 | 要清除的矩阵ID |
...
- **行数检查**：`rows <= settings_max_row && rows > 0`
- **列数检查**：`cols <= settings_max_col && cols > 0`

如果维度超限：
- 对于**命名矩阵**：触发清除操作，清除已写入的元数据
- 对于**匿名矩阵**：直接进入错误状态（未分配槽位）

### 数值范围验证

在写入每个数据元素时进行验证：

- **范围检查**：`settings_data_min <= data <= settings_data_max`
...
3. **维度超限**：行数或列数超过 settings 设置的最大值
4. **数值超限**：矩阵数据超出 settings 设置的范围

错误处理机制：
- 错误信号一旦拉高会持续保持，直到模块复位
- 检测到错误后，会自动触发清除操作（如果已开始写入）
- 清除操作将矩阵元数据（rows, cols, name）全部写为0
