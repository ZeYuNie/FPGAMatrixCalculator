# 矩阵加法模块 (matrix_op_add)

## 1. 功能概述

本模块执行两个矩阵的逐元素加法运算 ($C = A + B$)。结果矩阵将自动写入到系统预留的结果缓冲区（Matrix ID = 0）。
...
| `matrix_a_id` | input | 3 | 加数矩阵 A 的 ID (1-7) |
| `matrix_b_id` | input | 3 | 加数矩阵 B 的 ID (1-7) |
...
| `actual_rows` | output | 8 | 结果矩阵行数 |
| `actual_cols` | output | 8 | 结果矩阵列数 |
| `matrix_name` | output | 8x8 | 结果矩阵名称 ("ADDRES") |
...
4. **维度校验**: 验证 A 和 B 的行、列数是否完全相等。
5. **写入请求**: 向仲裁器请求写入权限。
6. **计算与写入**: 逐个读取 A、B 元素，计算和，并流式写入结果缓冲区。
...
- 结果矩阵名称固定为 "ADDRES"。
- 运算过程中会自动处理 BRAM 的读写时序（包括读取操作的 1 个时钟周期延迟）。
